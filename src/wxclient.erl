...
%%  ListBoxSizer = wxStaticBoxSizer:new(?wxVERTICAL, SubSizer1, [{label, "wxListBox"}]),
  ...
  ListBox = wxListBox:new(Frame, 1, [{size, {-1,100}},
    {choices, Choices}, {style, ?wxLB_SINGLE}]), %%TODO: cancel multiple selection
  wxListBox:setToolTip(ListBox, "A wxListBox with multiple selection"),

  ComboBox = wxComboBox:new(Frame, 5, [{choices, Choices}]),
  ...
  wxSizer:add(SubSizer2, ListBox, [{flag, ?wxALL bor ?wxEXPAND}, {border, 8}]),
  wxSizer:add(SubSizer2, ComboBox, [{flag, ?wxALL bor ?wxEXPAND}, {border, 8}]),
